        ...
        Try
            ...
            For intRow As Integer = 1 To oRowsMatrix.RowCount
                ...
                If dblSelectedqty > 0 Then
                    strqry = "select DistNumber FROM OBTQ T0  INNER JOIN OBTN T1 ON T0.AbsEntry = T1.AbsEntry INNER JOIN OITM T2 ON "
                    strqry = strqry & " T0.ItemCode = T2.ItemCode where T2.ItemCode='" & strItemCode & "' and  T0.[Quantity] > 0 order by T1.ExpDate,T1.SysNumber asc "
                    ...
                    For intLoop As Integer = 0 To oSerialRec.RecordCount - 1
                        ...
                        If diffQuantity >= 0 Then
                            For intloop1 As Integer = 1 To oSerialMatrix.RowCount
