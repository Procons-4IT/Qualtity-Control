Public Class clsQCControl
    ...
    Private Sub LoadForm()
        oForm = oApplication.Utilities.LoadForm(xml_QCReciving, frm_QCReceiving)
        oForm = oApplication.SBO_Application.Forms.ActiveForm()
        oForm.Freeze(True)
        oForm.DataBrowser.BrowseBy = "4"
        '  oForm.EnableMenu(mnu_ADD_ROW, True)
        ' oForm.EnableMenu(mnu_DELETE_ROW, True)
        oDataSrc_Line = oForm.DataSources.DBDataSources.Item("@Z_QCL1")
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_QCReceiving Then
...
#End Region
    ...
    Private Sub CreateStockTransfer(ByVal aform As SAPbouiCOM.Form)
        ...
        Dim str As String
        str = "Select * from [@Z_OQCL] where  isnull(U_Z_STNO,'')='' and    DocEntry=" & CInt(oApplication.Utilities.getEdittextvalue(aform, "4"))
        oRec.DoQuery(str)
        If oRec.RecordCount > 0 Then
            ...
            If str = "A" Then
                ...
                oRec.DoQuery("Select * from [@Z_QCL1] where U_Z_STWhsCode<>U_Z_QCWhsCode  and  DocEntry=" & CInt(oApplication.Utilities.getEdittextvalue(aform, "4")))
                If oRec.RecordCount > 0 Then
                    ...
                    For intRow As Integer = 0 To oRec.RecordCount - 1
                        ...
                        oSt.Comments = "Created based on Quality Control RM : " & CInt(oApplication.Utilities.getEdittextvalue(aform, "4"))
                        oRec.MoveNext()
                    Next
                    If oSt.Add <> 0 Then
                    ...
                    Else
                        ...
                        oRec.DoQuery("update [@Z_OQCL] set U_Z_STNO=" & st1 & " where DocEntry=" & CInt(oApplication.Utilities.getEdittextvalue(aform, "4")))
                        oApplication.Utilities.Message("Operation completed successfully", SAPbouiCOM.BoStatusBarMessageType.smt_Success)
                        '  oRec.DoQuery("Update OWTR set U_Z_QCDocNum='" & st1 & "' where DocEntry=" & st)
                    End If
                End If
            End If
    ...
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD) Then
                oForm = oApplication.SBO_Application.Forms.ActiveForm()
                If oForm.TypeEx = frm_QCReceiving Then
                    oForm = oApplication.SBO_Application.Forms.ActiveForm()
                    oForm.Items.Item("4").Enabled = False
                    oForm.Items.Item("6").Enabled = False
                    oForm.Items.Item("8").Enabled = False
